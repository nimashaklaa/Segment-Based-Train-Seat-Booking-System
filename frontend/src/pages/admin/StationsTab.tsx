...
export default function StationsTab() {
  const { stations, loading, saving, error, setError, load, create, update } = useStationsStore()
  ...
  function openEdit(s: (typeof stations)[0]) {
    setEditingId(s.id)
    setForm({
      name: s.name,
      sequence_order: String(s.sequence_order),
  ...
  async function handleSave() {
    ...
    if (isNaN(seq) || seq < 1) {
      setError('Sequence order must be a positive integer')
      return
    }
    ...
    try {
      if (editingId) {
        await update(editingId, form.name.trim(), seq, String(dist))
      } else {
        await create(form.name.trim(), seq, String(dist))
      }
      cancel()
    } catch {
      /* error set by store */
        ...
        {showForm && (
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm mb-5 p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <div>
                <label className={LABEL_CLS}>
                  Distance from Origin (km) <span className="text-red-500">*</span>
                </label>
                <input
                  type="number"
                  min={0}
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, distance_from_origin_km: e.target.value }))
                  }
