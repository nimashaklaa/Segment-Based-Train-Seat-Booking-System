...
interface Form { name: string; code: string; origin: string; destination: string }
...
export default function RoutesTab() {
  ...
  async function save() {
    if (!form.name.trim() || !form.origin.trim() || !form.destination.trim()) {
      setFormError('Name, origin and destination are required'); return
    }
    setSaving(true); setFormError('')
    try {
      const body = { name: form.name.trim(), code: form.code.trim(), origin: form.origin.trim(), destination: form.destination.trim() }
      if (editingId) { await api.updateRoute(editingId, body) } else { await api.createRoute(body) }
      cancel(); await load()
    } catch (err: unknown) {
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm mb-5 p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              <div>
                ...
                <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Colombo–Badulla" className={INPUT_CLS} />
              </div>
              <div>
                <label className={LABEL_CLS}>Code</label>
                <input type="text" value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} placeholder="e.g. CBE" className={INPUT_CLS} />
              </div>
              <div>
                <label className={LABEL_CLS}>Origin <span className="text-red-500">*</span></label>
                <input type="text" value={form.origin} onChange={e => setForm(f => ({ ...f, origin: e.target.value }))} placeholder="e.g. Colombo Fort" className={INPUT_CLS} />
              </div>
              <div>
                <label className={LABEL_CLS}>Destination <span className="text-red-500">*</span></label>
                <input type="text" value={form.destination} onChange={e => setForm(f => ({ ...f, destination: e.target.value }))} placeholder="e.g. Badulla" className={INPUT_CLS} />
              </div>
            </div>
                ...
                {routes.length === 0 ? (
                  ...
                  <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3 text-right">
                      <button onClick={() => openEdit(r)} className="inline-flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 px-2 py-1 rounded hover:bg-blue-50 transition-colors">
                        <Pencil size={11} /> Edit
                      </button>
                    </td>
                  </tr>
